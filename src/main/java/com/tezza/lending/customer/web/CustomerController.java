...
public class CustomerController {
    ...
    @GetMapping
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "List customers (paginated, optional status filter)")
    public ResponseEntity<ApiResponse<Page<CustomerResponse>>> list(
            @RequestParam(required = false) CustomerStatus status,
            @PageableDefault(size = 20, sort = "createdAt") Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.success(customerService.listCustomers(status, pageable)));
    }
